...
public class IncomeReportShowDataActivity extends AppCompatActivity {
    ...
    @SuppressLint("NonConstantResourceId")
    @BindView(R.id.rv_incomereport)
    RecyclerView rv_incomereport;
    ...
    private void incomeReportResponseCall() {
        avi_indicator.setVisibility(View.VISIBLE);
        avi_indicator.smoothToShow();
        RestApiInterface apiInterface = APIClient.getClient().create(RestApiInterface.class);
        Call<IncomeReportResponse> call = apiInterface.incomeReportResponseCall(RestUtils.getContentType(), reportDataRequest());
        Log.w(TAG,"IncomeReportResponse url  :%s"+" "+ call.request().url().toString());
        ...
        call.enqueue(new Callback<IncomeReportResponse>() {
            ...
            public void onResponse(@NonNull Call<IncomeReportResponse> call, @NonNull Response<IncomeReportResponse> response) {
                ...
                Log.w(TAG,"IncomeReportResponse" + new Gson().toJson(response.body()));
                if (response.body() != null) {

                    if (200 == response.body().getCode()) {
                        if(response.body().getData() != null && response.body().getData().size()>0){
                            txt_no_records.setVisibility(View.GONE);
                            rv_incomereport.setVisibility(View.VISIBLE);
                            setIncomeReportListView(response.body().getData());

                        }else{
                            rv_incomereport.setVisibility(View.GONE);
    ...
    private void setIncomeReportListView(List<IncomeReportResponse.DataBean> data) {
        rv_incomereport.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        rv_incomereport.setItemAnimator(new DefaultItemAnimator());
        IncomeReportListAdapter incomeReportListAdapter = new IncomeReportListAdapter(getApplicationContext(), data);
        rv_incomereport.setAdapter(incomeReportListAdapter);
